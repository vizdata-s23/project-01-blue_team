...
q1_DR <- ggplot(q1_viz1,
  ...
  theme(
    ...
    axis.title.y = element_text(hjust = -6.2),
...
```{r q1-viz2-plot}
ggplot(q1_viz2, aes(x = fct_rev(cocoa_grp),
                    y = avg_rating,
                    color = country_of_bean_origin)) +
  ...
  labs(title = "Chocolate Bar Ratings By Cocoa Percentage",
       subtitle = "By Country of Cocoa Bean Origin",
       x = "Cocoa Percentage",
       y = "Avg Rating (Out of 4)") +
  scale_color_manual(values = color_scheme) +
...
Next, to visualize how the ratings of different chocolate bar ingredients have changed over time, a single line plot will be used and faceted by color, with each line representing a different ingredient. A line plot is an effective way to visualize a relationship over time, since the intuitive nature of the lines from left to right tracks the variable over time. The mean ratings will be calculated for each ingredient for each year in the data set to construct the lines. Color mapping will be used as an easy way to identify the different ingredients and compare their mean ratings on the same plot and how they have changed over time.
